    ...
    render() {
        return (
            <>
                <Card className="m-2">
                    <Card.Body>
                        <Card.Title>Gene Search</Card.Title>
                        <Form noValidate validated={false}>
                            <InputGroup>
                                ...
                                />
                                <Button onClick={(e: any) => this.submit(e)} type="submit">
                                ...
                                <Accordion.Item eventKey="0">
                                    ...
                                    <Accordion.Body>
                                        {Array.from(this.props.highlightedGenes).map((gene: string) => {
                                            return (
                                                <Badge className="m-1" onClick={() => this.remove(gene)}>
                                                    {gene} <span className="bi bi-x"></span>
                                                </Badge>
                                            );
                                        })}
                                    </Accordion.Body>
                        ...
                        <Row>
                            <div className="mt-2 text-center">
                                <Button
                                    variant="success"
                                    className="me-2"
                                    disabled={!this.props.highlightMode}
                                    onClick={() => this.addGlobalSelection()}
                                >
                                    Add from current selection <i className="bi bi-box-arrow-right lg"></i>
                                </Button>
                                <Button
                                    variant="primary"
                                    className="me-2"
                                    disabled={!this.props.highlightMode}
                                    onClick={() => this.applyToGlobalSelection()}
                                >
                                    Apply to global selection <i className="bi bi-box-arrow-in-right"></i>
                                </Button>
                                <Button variant="danger" className="md-2" disabled={!this.props.highlightMode} onClick={() => this.resetSearch()}>
                                    Reset multi-search <i className="bi bi-trash"></i>
                                </Button>
                            </div>
                        </Row>
                    </Card.Body>
