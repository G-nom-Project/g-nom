        ...
        return (
            <>
                <InputGroup className="m-2">
                    <Form.Select onChange={(e: any) => this.setState({ new_id: parseInt(e.target.value) })} defaultValue={undefined}>
                        {this.state.analyses &&
                            this.state.analyses.map((e: Analysis) => {
                                return (
                    ...
                    <Button type="submit" onClick={() => this.props.dataset_changed(this.state.new_id)} disabled={this.state.new_id === -1}>
                        <span className="bi bi-arrow-right-circle m-2" />
                        Load
                    </Button>
                </InputGroup>
            </>
