...
const custom_color_generator = (item_pos: number, max_item_number: number, color_descriptor: string) => {
    if (color_descriptor == 'spectrum') {
        ...
        if (item_pos % 2 === 0) {
            return my_scale(item_pos / max_item_number)
                .saturate(3)
                .hex();
        } else {
            return my_scale(1 - item_pos / max_item_number)
                .saturate(3)
                .hex();
        }
...
class Scatter3D extends Component<Props, State> {
    constructor(props: Props) {
        ...
        this.state = {
            ...
            color_palette: 'colorblind', // currently selected color palette
            color_dict: {},
            ...
            hoverTemplate:
                '%{customdata[0]} <br>%{customdata[1]} <br><extra>Best hit: %{customdata[2]} <br>Best hit e-value: %{customdata[3]} <br>Taxonomic assignment: %{customdata[4]} <br>Contig name: %{customdata[5]} <br> </extra>',
            starsign_steps: 0,
            opacity: 1,
            trace_order: [],
            camera: {},
    ...
    componentDidUpdate(prevProps: Props, prevState: State) {
        // Dataset has changed
        if (prevProps.scatterPoints !== this.props.scatterPoints || prevProps.dim_string !== this.props.dim_string) {
            const new_size = this.set_auto_size(this.props.scatterPoints);
            this.setState({ marker_size: new_size, auto_size: true, starsign_steps: 0, legendonly: [], figure: {} }, () => {
                return this.build_plot();
            });
        }
        // Filters have changed
        else if (
            prevProps.e_value !== this.props.e_value ||
            prevProps.show_unassigned !== this.props.show_unassigned ||
            this.state.g_search_len !== this.props.g_searched.length ||
            prevProps.c_searched !== this.props.c_searched
        ) {
            return this.build_plot(undefined, undefined, true);
        }
        // gene order and gene selection
        else if (
            prevProps.selected_row !== this.props.selected_row ||
            prevState.starsign_steps !== this.state.starsign_steps ||
    ...
    shouldComponentUpdate(nextProps: Readonly<Props>, nextState: Readonly<State>): boolean {
        // New dataset
        if (
            nextProps.scatterPoints !== this.props.scatterPoints ||
            nextProps.dataset_id !== this.props.dataset_id ||
            nextProps.dim_string !== this.props.dim_string
        ) {
            return true;
        }
        // external changes
        if (
            nextProps.e_value !== this.props.e_value ||
            nextProps.show_unassigned !== this.props.show_unassigned ||
            nextProps.c_searched !== this.props.c_searched
        ) {
            return true;
        }
        // figure config changes => sync to scatter matrix as well
        if (
            nextState.color_dict !== this.state.color_dict ||
            nextState.opacity !== this.state.opacity ||
            nextState.marker_size !== this.state.marker_size ||
            nextState.legendonly !== this.state.legendonly
        ) {
            console.log(nextState.color_dict);
            this.props.passScatterData({
                colors: nextState.color_dict,
                legendonly: nextState.legendonly,
                opacity: nextState.opacity,
                marker_size: nextState.marker_size,
            });
            return true;
        }
        ...
        if (
            (nextProps.selected_row !== this.props.selected_row && this.state.starsign_steps > 0) ||
            nextState.starsign_steps !== this.state.starsign_steps
        ) {
            return true;
        }
        // We need to update the state in order to update the button color
        if (nextState.frozen_starsign_gene != this.state.frozen_starsign_gene) {
            return true;
        }
        if (nextProps.is_loading !== this.props.is_loading) {
            return true;
        }
        return false;
    }
    ...
    transformData(data: any[], isolated?: number, doubleclicked?: number, noSort?: boolean): any[] {
        ...
        if (!noSort && this.props.e_value === 1 && my_contigs.length === 0) {
            ...
            traces.sort(function (a, b) {
                return occurrences[b.text as keyof typeof occurrences] - occurrences[a.text as keyof typeof occurrences];
            });
            const new_order = [];
            ...
            traces.sort((a, b) => {
                return this.state.trace_order.lastIndexOf(a.text) - this.state.trace_order.lastIndexOf(b.text);
            });
            for (const trace of traces) {
                trace.name = trace.name + ` (${occurrences[trace.name as keyof typeof occurrences]})`;
            }
        ...
        for (let i = 0; i < traces.length; i++) {
            ...
            // @ts-ignore
            const next_color = custom_color_generator(i, traces.length, this.state.color_palette);
            traces[i]['marker']['color'] = next_color;
            my_color_dict[traces[i].text as string] = next_color;
            if (doubleclicked) {
                if (i !== doubleclicked) {
        ...
        if (this.state.starsign_steps > 0) {
            ...
            const ordered_genes = [];
            ordered_genes.push(center_gene);
            ...
            const gene_plot_hover_template =
                '%{customdata[1]}:%{customdata[2]}-%{customdata[3]}<br>Best hit: %{customdata[4]} <br> e-value: %{customdata[5]}';
            const e_value_x = [];
            const e_value_y = [];
            ...
            for (const gene of ordered_genes) {
                ...
                neighbouring_custom_data.push([
                    gene['plot_label'],
                    gene['g_name'],
                    gene['best_hit'],
                    gene['bh_evalue'],
                    gene['taxon_assignment'],
                    gene['c_name'],
                ]);
            }

            // Create lines connecting neighbouring genes
            console.log(all_genes_ordered);
            console.log(ordered_genes);
    ...
    build_plot(isolated?: number, doubeclicked?: number, initialBuild?: boolean): boolean {
        ...
        this.setState({
            figure: { data: new_data[0], layout: new_layout, config: new_config },
            g_search_len: this.props.g_searched.length,
            color_dict: new_data[1],
        });
        return true;
    }
        ...
        return (
            <>
                <Plot
                    ...
                    onInitialized={(e: any) => this.passCameraData(e)}
                ...
                <Row>
                    <Col xs={2}>
                        ...
                        <InputGroup>
                            <ButtonGroup>
                                <Button variant={this.state.hover_buttons[0]} onClick={() => this.switchHoverData('full')}>
                                    <span className="bi bi-eye-fill" />
                                </Button>
                                <Button variant={this.state.hover_buttons[1]} onClick={() => this.switchHoverData('reduced')}>
                                    <span className="bi bi-eye" />
                                </Button>
                                <Button variant={this.state.hover_buttons[2]} onClick={() => this.switchHoverData('none')}>
                    ...
                    <Col xs={1}>
                        ...
                        <InputGroup>
                            <Button onClick={() => this.exportVisible()}>
                                View
                                <span className="bi bi-box-arrow-in-right ml-1" />
                            </Button>
                        </InputGroup>
                    </Col>
                    <Col>
                        <Form.Label className="md-2">Gene order (steps)</Form.Label>
                        <InputGroup className="md-2">
                            <Button
                                disabled={!this.props.gene_order_supported}
                                onClick={() => {
                                    if (this.state.starsign_steps > 0) {
                                        this.setState({ starsign_steps: this.state.starsign_steps - 1 });
                                    }
                                }}
                            >
                            ...
                            <Button
                                disabled={!this.props.gene_order_supported}
                                onClick={() => this.setState({ starsign_steps: this.state.starsign_steps + 1 })}
                            >
                                <span className="bi bi-plus-circle"></span>
                            </Button>
                            <Button
                                variant="danger"
                                disabled={!this.props.gene_order_supported || this.state.starsign_steps === 0}
                                onClick={() => this.setState({ starsign_steps: 0 })}
                            >
                                <span className="bi bi-x-circle"></span>
                            </Button>
                            <Button
                                variant={(this.state.frozen_starsign_gene === '' && 'success') || 'danger'}
                                disabled={!this.props.gene_order_supported}
                                onClick={() => this.freezeStarsign(this.props.selected_row)}
                            >
                                <span className="bi bi-snow"></span>
                            </Button>
                        </InputGroup>
                    </Col>
                    <Col xs={3}>
                        ...
                        <InputGroup className="md-2">
                            <Button
                                disabled={this.state.auto_size}
                                onClick={() => {
                                    if (this.state.manual_size >= 2) {
                                        this.set_manual_size(this.state.manual_size - 1);
                                    }
                                }}
                            >
                                <span className="bi bi-dash-circle"></span>
                            ...
                            <Form.Control
                                ...
                                style={{ width: '15px' }}
                            />
                            <Button disabled={this.state.auto_size} onClick={() => this.set_manual_size(this.state.manual_size + 1)}>
                                <span className="bi bi-plus-circle"></span>
                            </Button>
                            <Button onClick={() => this.toggle_auto_size()} variant={(this.state.auto_size && 'success') || 'secondary'}>
                                <span className={(this.state.auto_size && 'bi bi-lock-fill') || 'bi bi-unlock-fill'}></span>Auto
                            </Button>
                        </InputGroup>
                    </Col>
                    <Col xs={2}>
                    ...
                    <Col xs={2}>
                        ...
                        <Form.Select defaultValue={'colorblind'} onChange={(e: any) => this.set_color_palette(e.target.value)}>
