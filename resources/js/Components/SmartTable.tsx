...
interface Props {
    row_keys: string[];
    col_keys: ColumnKey[];
    master_data: GenericRow[];
    searchable_keys: string[];
    // eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
    passClick: Function | undefined;
}

interface State {
    table_data: GenericRow[];
    key: string;
    loading: boolean;
...
class SmartTable extends Component<Props, State> {
    ...
    sortRows(rows: GenericRow[]) {
        ...
        return [...rows].sort((a, b) => {
            ...
            // Detect numeric
            const aNum = parseFloat(av as string);
            const bNum = parseFloat(bv as string);
    ...
    filterRows(rows: GenericRow[]) {
        const { search } = this.state;

        return rows.filter((row: GenericRow) =>
    ...
    componentDidUpdate(prevProps: Props) {
        if (prevProps.row_keys !== this.props.row_keys) {
            const rows = this.props.master_data;
            this.setState({ rows: rows });
        }
    }

    render() {
        const baseRows = this.props.master_data;
            ...
            <>
                <div className="table-responsive mt-2">
                    <Table striped bordered hover>
                        <thead>
                            <tr>
                                {this.props.passClick && <th>Select</th>}
                                {this.props.col_keys.map((each) => (
                                    <th key={each.value}>
                                        <button type="button" className="btn btn-link p-0" onClick={() => this.handleSort(each.value)}>
                                            {each.label}
                                            {this.state.sortKey === each.value && (this.state.sortDir === 'asc' ? ' ▲' : ' ▼')}
                                        </button>
                                        {(this.props.searchable_keys.includes(each.value)) && (
                                            <input
                                                type="text"
                                                className="form-control form-control-sm mt-1"
                                                placeholder="Search…"
                                                value={this.state.search[each.value] || ''}
                                                onChange={(e) => this.handleSearchChange(each.value, e.target.value)}
                                            />
                                        )}
                                    </th>
                                ))}
                        ...
                        <tbody>
                            {pagedRows &&
                                pagedRows.map((each, idx) => (
                                    <tr key={idx}>
                                        {this.props.col_keys.map((field) => (
                                            <td key={field.value}>{each[field.value] ?? 'No data'}</td>
                                        ))}
                                    </tr>
                                ))}
                        </tbody>
